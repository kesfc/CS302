...
# Masks provided by the rest of the code assume the 'head' or important
# feature is drawn at the top of the array (row 0).  The simulator wants
# the bottom of the mask to sit on the ground, so we vertically flip the
# mask here before building the geometry.  That way you can design
# patterns naturally (head up) and they will appear upright in the sim.
#
# This also covers the hand‑coded pokemon seeds, which were written with
# their heads at the top of the grid; previously those ended up upside
# down because the flip only occurred in sample_mask.
def mask_to_robot(mask):
    # flip mask vertically so row 0 becomes the bottom row
    mask = np.flipud(mask)
...
def sample_mask(p):
    ...
    # Place component at the bottom, flipped vertically so top is at bottom
